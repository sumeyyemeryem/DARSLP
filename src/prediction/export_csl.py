"""
Export DARSLP Predictions — CSL-Daily Test Set
================================================
Runs inference with a trained DARSLPGenerator + DisentangledAE checkpoint on the
CSL-Daily test split and saves all predictions to a .npz archive.

The output file is compatible with the SLT back-translation evaluation pipeline
(https://github.com/neccam/slt).

Output .npz keys:
    names      — (N,)  dataset-specific sample IDs  (e.g. "S000020_P0004_T00")
...
Note on wrist slot convention (CSL-Daily):
    ...
    PHOENIX-2014T. The joint insertion here mirrors the original CSL export:
      LHWrist (index 27) → RWrist slot (index 2)
      RHWrist (index  6) → LWrist slot (index 5)

Usage:
    python src/prediction/export_csl.py \\
        ...
        --output          exports/predictions_DARSLP_CSL_test.npz
"""
...
def main(args):
    ...
    vocab  = AutoTokenizer.from_pretrained("hfl/chinese-bert-wwm").vocab
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output",          default=r"exports\predictions_DARSLP_CSL_test.npz")
